...
export default {
  ...
  methods: {
    ...
    initDate: function (markDate) {
      ...
      let that = this
      let today = new Date()
      let cur = markDate ? new Date(markDate) : today // 初始日期
      let cmon = cur.getMonth() + 1 // 月份
      let cyear = cur.getFullYear() // 年份
      this.today = that.formatDate(
        today.getFullYear(),
        today.getMonth() + 1,
        today.getDate()
      )
      return that.render(cyear, cmon)
    },
    render: function (y, m) {
      // 渲染内容  m=1~12
      this.curY = y
      ...
      let sumDay = new Date(y, m, 0).getDate() // 当月总天数
      let lastSumDay = new Date(y, m - 1, 0).getDate() // 上月总天数
      let week = cur.getDay() // 本月1号周几
