<template>
  <div class="chart">
    ...
    <div class="barBox" draggable="true">
      <ul class="checker">
        <li v-for="item in chartType" @click="creatFloatChart('floatChart',item.type)" :key="item.type">{{ item.name }}</li>
      </ul>
      <div class="barChart" id="floatChart"></div>
    </div>
    <div id="pieDom" class="newbarmap" v-show="currentScatter!==''" :style="pieDomStyle">根据map的散点显示数据</div>
    <div id="mapZoom">
...
export default {
  ...
  mounted: function () {
    this.$ajax.get('/json/mapdata.json').then(res => {
